...
def test_recon_logic():
    ...
    # 1. Fetch latest shift and CSA
    shift_name = frappe.db.get_value("Shift", {}, "name", order_by="creation desc")
    csa_id = frappe.db.get_value("Shift Assigned CSA", {"parent": shift_name}, "csa")
    if not shift_name or not csa_id:
        print("No shift or CSA found in database to run reconciliation test!")
        return "SKIPPED"
    ...
    try:
        ...
        recon_data = get_csa_reconciliation_data(shift_name, csa_id)
        
        print(f"Customer Payments Liability sum: {recon_data['customer_payments']}")
        print(f"M-Pesa Deduction sum: {recon_data['mpesa']}")
        
        # Assertions
        payments_list = [p.name for p in recon_data['customer_payments_breakdown']]
        assert payment_id in payments_list, "Test payment not found in customer payments list!"
        print("Reconciliation integration logic checks passed.")
    finally:
        # Clean up
        frappe.delete_doc("Customer Payment", payment_id)
        print(f"Deleted test payment: {payment_id}")
    
    print("SUCCESS: Reconciliation integration logic test passed successfully!")
    return "SUCCESS"
